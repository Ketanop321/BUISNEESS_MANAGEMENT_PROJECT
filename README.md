...
Use this when you want to run quickly without installing MySQL first.

1. Install JDK 17 and verify with `java -version`. <br>
2. Run the app with local profile : `set SPRING_PROFILES_ACTIVE=local` and then `mvnw.cmd spring-boot:run` (Windows CMD) <br>
   - For PowerShell: `$env:SPRING_PROFILES_ACTIVE='local'; .\\mvnw.cmd spring-boot:run` <br>
3. Open http://localhost:2330/home <br>
4. Optional DB UI for local profile: http://localhost:2330/h2-console <br>

This profile uses in-memory H2 so startup works even when MySQL is not running.


## Git Ownership Cleanup (Professional Note) :

For sharing project code without previous ownership history:

1. Remove old Git metadata: delete `.git` folder (already done in this workspace). <br>
2. Initialize a fresh repository: `git init` <br>
3. Add your own remote: `git remote add origin <your-repo-url>` <br>

In industry, deleting history is usually avoided for team projects. Preferred approach is preserving history and using proper repository transfer or access control.
